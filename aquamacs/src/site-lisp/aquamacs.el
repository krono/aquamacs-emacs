...
(defun aquamacs-setup ()
  ...
  ;; XXX This doesn't work as advertised, so we need to figure
  ;; out how to make it work, or some reasonable other change.
  ;; Leaving the code here now, guarded by the when-nil--WT.
  ...
  (define-key global-map [remap scroll-up]           'aquamacs-page-down)
  ...
  (define-key global-map [remap scroll-down]         'aquamacs-page-up)
...
to the selected frame."
  ...
  (defun aquamacs-advise--cua-copy-region (orig-fun &rest args)
    "Advise cua-copy-region to keep the region when saving to clipboard.
Arguments passed through to original."
    ;; respect user's setting of cua-keep-region-after-copy for M-w etc.
    (let ((cua-keep-region-after-copy
           (or (eq this-original-command 'clipboard-kill-ring-save)
               cua-keep-region-after-copy)))
      (apply orig-fun args)))
  ;; XXX TEMP COMMENT OUT FOR DEBUGGING LOADUP XXX
  ;; XXX TRY AGAIN BUT LEAVING XXX here for now
  (advice-add 'cua-copy-region :around #'aquamacs-advise--cua-copy-region)
